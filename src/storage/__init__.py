        ...
        self.backup_service = Container("backup-service",
            ...
            command=[
                ...
                while true; do
                    ...
                    # Jenkins backup with compression
                    echo "Backing up Jenkins..."
                    pigz -c /jenkins_home > $BACKUP_DIR/jenkins.tar.gz
                    
                    # Nginx configs
                    echo "Backing up Nginx configs..."
                    tar czf $BACKUP_DIR/nginx.tar.gz /nginx_data
                    ...
                    curl -X PUT "http://elasticsearch:9200/_snapshot/backup/$BACKUP_DATE?wait_for_completion=true"
                    
                    # Grafana
                    echo "Backing up Grafana..."
                    tar czf $BACKUP_DIR/grafana.tar.gz /grafana_data
                    
                    # Prometheus data
                    echo "Backing up Prometheus data..."
                    tar czf $BACKUP_DIR/prometheus.tar.gz /prometheus_data
                    ...
                    cat > $BACKUP_DIR/manifest.json << EOF
                    {
                        ...
                        "components": {
                            "jenkins": {"size": "$(du -sh $BACKUP_DIR/jenkins.tar.gz | cut -f1)"},
                            "nginx": {"size": "$(du -sh $BACKUP_DIR/nginx.tar.gz | cut -f1)"},
                            "elasticsearch": {"size": "$(du -sh /backup/elasticsearch | cut -f1)"},
                            "grafana": {"size": "$(du -sh $BACKUP_DIR/grafana.tar.gz | cut -f1)"},
                            "prometheus": {"size": "$(du -sh $BACKUP_DIR/prometheus.tar.gz | cut -f1)"}
                        }
                    }
                    EOF
                    
                    # Archive old backups (older than 7 days) to separate volume
                    echo "Archiving old backups..."
                    find /backup -maxdepth 1 -type d -mtime +7 -exec mv {} /archive/ \;
                    
                    # Cleanup old archives (older than 30 days)
                    echo "Cleaning up old archives..."
                    find /archive -maxdepth 1 -type d -mtime +30 -exec rm -rf {} \;
                    
                    # Generate backup report
                    echo "Generating backup report..."
                    cat > $BACKUP_DIR/report.txt << EOF
                    Backup Report - $BACKUP_DATE
                    =============================
                    Total Size: $(du -sh $BACKUP_DIR | cut -f1)
                    Components:
                    $(jq -r '.components | to_entries | .[] | "  - \(.key): \(.value.size)"' $BACKUP_DIR/manifest.json)
                    
                    Archive Status:
                    - Archived backups: $(find /archive -maxdepth 1 -type d | wc -l)
                    - Oldest archive: $(ls -tr /archive | head -1)
                    - Newest archive: $(ls -tr /archive | tail -1)
                    EOF
